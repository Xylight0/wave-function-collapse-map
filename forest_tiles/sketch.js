...
function preload() {
  for (let i = 1; i <= 4; i++) {
    tilesImages.push(loadImage(`tiles/${i}.jpg`));
  }
}
...
function setup() {
  ...
  tiles[0] = new Tile(tilesImages[0], ["B", "A", "A", "A"]);
  tiles[1] = new Tile(tilesImages[1], ["A", "A", "A", "A"]);
  tiles[2] = new Tile(tilesImages[2], ["B", "B", "A", "A"]);
  tiles[3] = new Tile(tilesImages[3], ["B", "A", "B", "A"]);
